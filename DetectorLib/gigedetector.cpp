...
int GigEDetector::initialise(Triggering triggering)
{
   ...
   CONST LPSTR deviceDescriptor = (const LPSTR )"148.79.208.138";
   //CONST LPSTR deviceDescriptor = (const LPSTR )"00:11:1C:01:EB:24";
   ...
   qDebug() << "Initialising detector:"<<deviceDescriptor<<":";
   ...
   qDebug() << "(DetLib)InitDevice returns " << status;
...
//      abort(true);
   ...
   else if (command == CLOSE)
   {
      ...
      {
         emit writeError("(DetLib/GigEDet)Failed to terminate detector connection, status = " + QString::number(status));
      }
   }
