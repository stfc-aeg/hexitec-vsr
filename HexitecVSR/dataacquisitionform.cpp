...
void DataAcquisitionForm::handleMonitorData(MonitorData *md)
{
   ...
   /* shk78
    * Replaced with handleTerminateDetector SLOT
    * Signal sent by detectorControlForm when monitoring fails completely
   else
   {
      disableGui();
   }*/
...
void DataAcquisitionForm::handleTerminatedDetector()
{
   printf("Handling termination of detector connection\n"); fflush(stdout);
   disableGui();
}
...
void DataAcquisitionForm::guiMode()
{
   ...
   int mode = VSRDetector::CONTINUOUS;
   switch (mode)
   {
   case VSRDetector::CONTINUOUS:
   ...
   case VSRDetector::VSR_DEFAULT:
   case VSRDetector::INVALID_MODE:
      ui->collectImages->setEnabled(false);
      ui->duration->setEnabled(false);
      ui->dataFileDirectory->setEnabled(false);
      ui->dataFileDirectoryButton->setEnabled(false);
