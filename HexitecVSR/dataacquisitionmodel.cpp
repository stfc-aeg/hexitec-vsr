...
void DataAcquisitionModel::connectDetectorMonitor()
{
   ...
   //shk78 connect(detectorMonitor, SIGNAL(writeError(QString)), ApplicationOutput::instance(), SLOT(writeError(QString)));
   connect(detectorMonitor, SIGNAL(writeError(QString)), detectorMonitor, SLOT(handleWriteError(QString)));
...
{
   connect(dataAcquisition, SIGNAL(executeCommand(VSRDetector::DetectorCommand, int, int)),
           vsrDetector, SLOT(handleExecuteCommand(VSRDetector::DetectorCommand, int, int)));
   connect(dataAcquisition, SIGNAL(executeOffsets()),
           vsrDetector, SLOT(handleExecuteOffsets()));
   connect(dataAcquisition, SIGNAL(executeReducedDataCollection()),
           vsrDetector, SLOT(handleReducedDataCollection()));
   ...
   connect(dataAcquisition, SIGNAL(setTargetTemperature(double)),
           vsrDetector, SLOT(handleSetTargetTemperature(double)));
   connect(dataAcquisition, SIGNAL(appendTimestamp(bool)),
           vsrDetector, SLOT(handleAppendTimestamp(bool)));
...
// shk78: Someone has been 'adjusting' signals/slots. This signal no longer exists.
//   connect(dataAcquisition, SIGNAL(imageStarting(char *, int, int)),
//           processingBufferGenerator, SLOT(handleImageStarting(char *, int, int)));
}

void DataAcquisitionModel::connectVSRDetector()
{
   connect(vsrDetector, SIGNAL(writeMessage(QString)), ApplicationOutput::instance(), SLOT(writeMessage(QString)));
   connect(vsrDetector, SIGNAL(writeError(QString)), ApplicationOutput::instance(), SLOT(writeError(QString)));
   connect(vsrDetector, SIGNAL(notifyState(VSRDetector::DetectorState)), dataAcquisition, SLOT(receiveState(VSRDetector::DetectorState)));
   connect(vsrDetector, SIGNAL(imageAcquired(QPixmap)), detectorControlForm, SLOT(setPixmap(QPixmap)));
   connect(vsrDetector, SIGNAL(prepareForOffsets()), dataAcquisitionForm, SLOT(prepareForOffsets()));
   connect(vsrDetector, SIGNAL(prepareForDataCollection()), dataAcquisitionForm, SLOT(prepareForDataCollection()));
// shk78: Someone has been 'adjusting' signals/slots. This slot no longer exists
//   connect(vsrDetector, SIGNAL(imageStarted(char *, int)),
//           dataAcquisition, SLOT(handleImageStarted(char *, int)));
   connect(vsrDetector, SIGNAL(imageStarted(char *)),
           processingBufferGenerator, SLOT(handleImageStarted(char *)));
   connect(vsrDetector, SIGNAL(imageComplete(unsigned long long)),
           dataAcquisition, SLOT(handleImageComplete(unsigned long long)));
//   connect(vsrDetector, SIGNAL(executeBufferReady(unsigned char*,ulong)),
//           dataAcquisition, SLOT(handleBufferReady(unsigned char*,ulong));
   connect(vsrDetector, SIGNAL(enableMonitoring()),
           detectorMonitor, SLOT(enableMonitoring()));
   connect(vsrDetector, SIGNAL(executeCommand(VSRDetector::DetectorCommand, int, int)),
           vsrDetector, SLOT(handleExecuteCommand(VSRDetector::DetectorCommand, int, int)));
   connect(vsrDetector, SIGNAL(triggeringAvailableChanged(bool)),
           detectorControlForm, SLOT(handleTriggeringAvailable(bool)));
//   connect(vsrDetector, SIGNAL(detectorResolutionSet(int, int)),
//           processingBufferGenerator, SLOT(handleConfigureSensor(int, int)));
   connect(vsrDetector, SIGNAL(cancelDataCollection()),
...
void DataAcquisitionModel::connectDetectorControlForm()
{
   ...
   connect(detectorControlForm, SIGNAL(executeCommand(VSRDetector::DetectorCommand, int, int)),
           vsrDetector, SLOT(handleExecuteCommand(VSRDetector::DetectorCommand, int, int)));
   ...
   connect(detectorControlForm, SIGNAL(setTriggerTimeout(double)),
           vsrDetector, SLOT(handleSetTriggerTimeout(double)));
   connect(detectorControlForm, SIGNAL(terminatedDetector()),
           dataAcquisitionForm, SLOT(handleTerminatedDetector()));
}
...
{
   connect(dataAcquisitionForm, SIGNAL(executeCommand(VSRDetector::DetectorCommand, int)),
           vsrDetector, SLOT(handleExecuteCommand(VSRDetector::DetectorCommand, int)));
   ...
   connect(dataAcquisitionForm, SIGNAL(saveRawChanged(bool)),
           vsrDetector, SLOT(handleSaveRawChanged(bool)));
}
...
void DataAcquisitionModel::connectHV()
{
   ...
   connect(hv, SIGNAL(setHV(double)), vsrDetector, SLOT(handleSetHV(double)));
}
