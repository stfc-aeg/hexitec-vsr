...
void DetectorControlForm::readIniFile()
{
   ...
   {
      detectorFilename = settings.value("hexitecVSRIniFilename").toString();
   }
...
void DetectorControlForm::terminateDetector()
{
   ...
   emit executeCommand(VSRDetector::CLOSE, 0, 0);
   emit terminatedDetector();
...
void DetectorControlForm::handleMonitorData(MonitorData *md)
{
   if (md->getValid())
   {
      ui->housingTemperature1->setText(QString::number(md->getTH(0), 'f', 1));
      ui->housingTemperature2->setText(QString::number(md->getTH(1), 'f', 1));
      ui->housingHumidity1->setText(QString::number(md->getRH(0), 'f', 1));
      ui->housingHumidity2->setText(QString::number(md->getRH(1), 'f', 1));
      ui->fingerTemperature->setText(QString::number(md->getT(), 'f', 1));
      ui->dewPoint1->setText(QString::number(md->getTDP(0), 'f', 1));
      ui->dewPoint2->setText(QString::number(md->getTDP(1), 'f', 1));
      ui->detectorTemperature1->setText(QString::number(md->getTASIC(0), 'f', 1));
      ui->detectorTemperature2->setText(QString::number(md->getTASIC(1), 'f', 1));
      ui->detectorTemperature3->setText(QString::number(md->getTASIC(2), 'f', 1));
      ui->detectorTemperature4->setText(QString::number(md->getTASIC(3), 'f', 1));
      if (firstMonitor)
      {
...
//         qDebug() << "DetectorControlForm::handleMonitorData(MonitorData *md)" << md->getT();
   ...
   else
   {
      ...
      else
      {
         ...
         printf("Time since last error %f\n", difftime(currentTime, lastErrorTime));
         if (difftime(currentTime, lastErrorTime) < 10.0)
         {
            errorCount++;
            if (errorCount > 3)
            {
               emit writeError("Monitor Error: terminating detector connection");
               terminateDetector();
            }
            else
            {
                printf("Error count = %d, will it recover?\n", errorCount); fflush(stdout);
            }
         }
         else
         {
            printf("Resetting last error time\n"); fflush(stdout);
            errorCount = 0;
            lastErrorTime = time(NULL);
         }
      }
...
{
   ui->imageLabel->setPixmap(pixmap.scaled(320, 320));
}
