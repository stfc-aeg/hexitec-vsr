...
//   qDebug() << " -=-=- pushRawFileName() " << aFileName.c_str() << "frameSize (bytes) = " << frameSize;
   ...
   this->prepSettings();
    ...
    /// HexitecVSR Addition, Check file has .hxt file ending - It always should have
...
void hexitech::HxtProcessing::pushTransferBuffer(unsigned char *transferBuffer, unsigned long validFrames)
{
   ...
   /// back to the VSRDetector thread for saving the raw data and for re-use.
...
//                emit hexitechSignalError("HxtProcessing Unable to acquire Motor mutex lock!");
        ...
        else
        {
            ...
            if (currentCondition == conditionEveryNewFile)         /// HexitecVSR: filenames replaced by buffers
            {
