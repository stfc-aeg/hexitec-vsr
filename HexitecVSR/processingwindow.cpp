...
//              processingWindowInstance, SLOT(handleHxtProcessedFileNameChanged(string)));
      ...
      /// HexitecVSR Addition: Signal when HDF5, CSV  file(s) (de)selected
      ...
      /// HexitecVSR Addition: Add slot so processingLoggingCheckBox toggles logging on/off
      ...
      /// HexitecVSR Addition: (The following 2 connections)
   ...
   ui(new Ui::ProcessingWindow)
{
   ...
   QSettings settings(QSettings::UserScope, "TEDDI", "HexitecVSR");
   if (settings.contains("hexitecVSRIniFilename"))
   {
      hexitechFilename = settings.value("hexitecVSRIniFilename").toString();
   }
...
void ProcessingWindow::initialiseProcessingWindow()
{
    ...
    /// itself using default settings from HexitecVSR.ini - Gui crashes
...
void ProcessingWindow::customCheckBoxToggled(bool isChecked)
{
    ...
    /// HexitecVSR Addition; Signal to HexitecVSR/HxtProcessing status of custom file selection
...
{
    /// HexitecVSR: Calibration options now: None (ADU) / Energy (KeV) / Momentum (including Energy; nm^-1)
    ...
    // Reload gui's Spectrum Options with default values from HexitecVSR.ini file
...
{
    /// HexitecVSR: Calibration options now: None / Energy / Momentum (including Energy)
    ///
    /// Update Spectrum Options with default values from hexitecVSR.ini file
...
void ProcessingWindow::hxtFileLineEditChanged(QString newPrefixString)
{
    ...
    {
        // It's a new filename, update HxtProcessing object and HexitecVSR.ini file object
...
void ProcessingWindow::hxtFileSaveButtonPressed()
{
    ...
    if (!newHxtFilename.toStdString().empty())
    {
        ...
        // Check file name valid and communicate changed name onto HexitecVSR.ini, HxtProcessing
...
{
    /// Prepare gradients and intercepts files to be saved into the hexitecVSR.ini file
...
{
    /// Extract Gradients and Intercepts filename from key "Calibration File" entry of HexitecVSR.ini file
...
void ProcessingWindow::saveSettingsButtonPressed()
{
    ...
    // Prepare to update CalibrationFile entry in hexitecVSR.ini
