										...
										u32 frameTimeOut )
{
	...
	IM_SET_BUSY(deviceHdl);

	IM_SET_READY(deviceHdl);
...
{				
	...
	if( dResult == AS_NO_ERROR )
	{
		dResult = deviceHdl->CloseSerialPort( 0 );
	}
										...
										p_u32 pleoraErrorDescriptionLen )
{
	...
	*internalResult = lResult.GetCode();
	...
	{
		iResult = AS_NTX_INIT_DEVICE_ERROR;
	}
...
{				
	i32 dResult = AS_NO_ERROR;
	NTxSerialPortConfig lPortConfig;

	lPortConfig.BaudRate = "Baud38400";
	lPortConfig.LoopBack = false;
	lPortConfig.Mode = "UART";
	lPortConfig.Parity = "None";
	lPortConfig.SerialPort = serialPort;
	lPortConfig.StopBits = "One";

	IM_SET_BUSY(deviceHdl);

	if( dResult == AS_NO_ERROR )
	{
		dResult = deviceHdl->OpenSerialPort( 0, &lPortConfig, rxBufferSize, useTermChar, termChar );
	}
										...
										u32 timeOut )
{
	...
	{
		dResult = deviceHdl->FlushRxBuffer( 0 );

		if( dResult == AS_NO_ERROR )
		{
			dResult = deviceHdl->WriteSerialPort( 0, txBuffer, txBufferSize, bytesWritten );
		}
	}

	if( (dResult == AS_NO_ERROR) && rxBufferSize )
	{
		dResult = deviceHdl->ReadSerialPort( 0, rxBuffer, rxBufferSize, bytesRead, timeOut );
	}
										...
										const str8 testPattern )
{
	...
	if( dResult == AS_NO_ERROR )
	{
		dResult = deviceHdl->SetImageFormatControl( pixelFormat, width, height, offsetX, offsetY, sensorTaps, testPattern, 1 );
	}
